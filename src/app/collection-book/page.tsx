...
const LoanManagement: React.FC = () => {
  ...
  const [receivedAmounts, setReceivedAmounts] = useState<{ [key: string]: number }>({});
  ...
  const [paymentHistoryCache, setPaymentHistoryCache] = useState<{ [key: string]: Payment[] }>({});
  const [loanDetailsCache, setLoanDetailsCache] = useState<{ [key: string]: LoanDetails }>({});
        ...
        case "Enter":
          if (currentColumn === "accountNo") {
            ...
            const isDuplicate = payments.some(
              (payment, idx) => idx !== currentRow && payment.accountNo === trimmedValue
            );

            if (isDuplicate) {
              setAlertMessage("This account number is already entered in another row.");
              setAlertOpen(true);
              inputRefs.current[`accountNo-${currentRow}`]?.focus();
              return;
            }
  ...
  const handleAmountPaidChange = (value: string, index: number) => {
    ...
    if (accountNo) {
      ...
      setReceivedAmounts((prev) => ({
        ...prev,
        [accountNo]: previousReceived + amount,
      }));
    }
  };
  ...
  const savePayments = async () => {
    ...
    try {
      const paymentData = {
        ...
        payments: validPayments.map((p) => {
          ...
          return {
            ...
            lateAmount: lateAmounts[p.accountNo] || 0, // Use existing late amount, will be updated later
      ...
      // Update payments with server response
      const updatedPayments = responseData.payments.map((payment: Payment, index: number) => ({
        ...payment,
        index: index + 1,
      ...
      // Set accounts that need refreshing
      const accountsToRefresh: string[] = updatedPayments.map((p: Payment) => p.accountNo);
      const currentAccountNo = updatedPayments[currentRow]?.accountNo || '';
  
      // Trigger useEffect to sync values
      setIsSaved({ 
        status: true, 
  ...
  // useEffect to ensure UI updates after save
  // useEffect to ensure UI updates after save
  useEffect(() => {
    if (isSaved.status) {
      const syncValues = async () => {
        ...
        await Promise.all(
          isSaved.accounts.map(async (accountNo) => {
            ...
            if (loanDetailsData) {
              ...
              // Calculate late amount consistently with calculateLateAmount
              ...
              if (!isNaN(loanDate.getTime())) {
                ...
                if (loanDetailsData.isDaily) {
                  ...
                  expectedPayments = monthsFromLoan * loanDetailsData.instAmount;
                }
  
                const lateAmount = expectedPayments - totalReceived;
                newLateAmounts[accountNo] = lateAmount;
  
                // Update payments with the new late amount
                setPayments((prevPayments) =>
        ...
        // Refresh loan details display for the current account
        if (isSaved.currentAccount && newLoanDetailsCache[isSaved.currentAccount]) {
        ...
        if (paymentToDelete._id) {
          ...
          if (response.ok) {
            ...
            if (updatedPayments.length === 0) {
              ...
              setPayments(updatedPayments.map((payment, idx) => ({
                ...payment,
                index: idx + 1,
              })));
  ...
  useEffect(() => {
    ...
    if (tableBody && selectedRow) {
      ...
      const scrollPosition = Math.max(0, currentRow * rowHeight - 2 * rowHeight);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.key.toLowerCase() === "d") {
        e.preventDefault();
        const datePickerInput = document.querySelector('input[placeholder="dd-MM-yyyy"]') as HTMLInputElement | null;
        if (datePickerInput) {
  ...
  const calculateLateAmount = async (details: LoanDetails, accountNo: string) => {
    ...
    try {
      const paymentHistory = paymentHistoryCache[accountNo] || await fetchPaymentHistory(accountNo);
      ...
      const sortedPayments = paymentHistory.sort(
        (a: Payment, b: Payment) => new Date(a.paymentDate as Date).getTime() - new Date(b.paymentDate as Date).getTime()
      );
    ...
    <div className={`${ubuntu.className} min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-2 md:p-4 transition-colors`}>
          ...
          <div className="flex flex-col lg:flex-row md:items-start lg:items-center gap-4 md:gap-8">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4">
              <label className="text-base items-center flex gap-2 md:text-lg font-semibold text-gray-700 dark:text-gray-300">
                Payment Date
                <span onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 p-2 bg-orange-500 hover:bg-orange-600 lg:hidden text-white rounded-full transition-colors">
                  <Search size={12} />
                </span>
              </label>
              <ReactDatePicker
            ...
            <div className="flex items-center justify-between md:justify-start gap-4 md:gap-8 lg:pl-28">
              {isDarkMode ? (
                <Image src="/GFLogo.png" alt="logo" height={50} width={50} className="w-8 lg:w-10 drop-shadow-[0_0_0_0.5] transition-opacity cursor-pointer" onClick={() => router.push("/")} />
              ) : (
                <Image src="/lightlogo.png" alt="logo" height={50} width={50} className="w-8 lg:w-10 drop-shadow-[0_0_0_0.5] transition-opacity cursor-pointer" onClick={() => router.push("/")} />
              )}
              ...
              <Link href="/loan" className="px-4 md:px-6 py-2 md:py-3 bg-orange-600 text-white rounded-xl text-sm md:text-base hover:bg-orange-700 transition-colors">
                Loans
              </Link>
            </div>
          </div>
          <div className="lg:flex justify-between items-center hidden lg:gap-12">
            <span className="text-base md:text-lg font-medium text-gray-600 dark:text-gray-400">{formatDate(selectedDate)}</span>
            <TimeDisplay />
          </div>
        </div>
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl md:rounded-2xl shadow-lg p-4 md:p-8 pt-4 md:pt-5 border border-gray-200/60 dark:border-gray-700">
            ...
            {loanDetails ? (
                ...
                <div className="space-y-3 md:space-y-3">
                  <div className="grid gap-2 md:gap-2">
                    {[
                      ...
                      { label: "Loan Date", value: new Date(loanDetails.date).toLocaleDateString("en-GB").replace(/\//g, "-") },
                      { label: "M. Date", value: new Date(loanDetails.mDate).toLocaleDateString("en-GB").replace(/\//g, "-") },
                      { label: "Period Type", value: formatPeriodType(loanDetails.isDaily) },
                      { label: "Total Amount", value: `₹${loanDetails.mAmount}` },
                      { label: "Installment", value: `₹${loanDetails.instAmount}` },
                    ].map((item) => (
                      <div key={item.label} className="flex justify-between items-center">
                        <span className="text-sm md:text-base font-medium text-orange-600">{item.label}</span>
                        <span className="text-base md:text-lg text-gray-900 dark:text-gray-100 font-semibold text-right uppercase">{item.value}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {payments[currentRow]?.accountNo && (
                    <div className="mt-4 p-4 md:p-6 bg-orange-50 dark:bg-orange-900/10 rounded-xl space-y-3 md:space-y-4 border border-orange-200 dark:border-orange-800">
                      <div className="grid gap-2 md:gap-3">
                        {[
                          { label: "Account No", value: payments[currentRow].accountNo },
                          { label: "Received Amount", value: `₹${receivedAmounts[payments[currentRow].accountNo] || 0}` },
                          { label: "Late Amount", value: `₹${lateAmounts[payments[currentRow].accountNo] || 0}` },
                          { label: "Total to be Paid", value: `₹${calculateTotalToBePaid(payments[currentRow].accountNo)}` },
                        ].map((item) => (
                          <div key={item.label} className="flex justify-between items-center">
                            <span className="text-sm md:text-base font-medium text-orange-700 dark:text-orange-300">{item.label}</span>
                            <span className="text-base md:text-lg text-orange-900 dark:text-orange-100 font-semibold">{item.value}</span>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="h-48 md:h-64 flex items-center justify-center">
                <p className="text-base md:text-lg text-gray-500 dark:text-gray-400">Select an account to view loan details</p>
              </div>
            )}
          </div>
        </div>
                ...
                <thead className="sticky top-0 z-50 bg-orange-50 dark:bg-orange-950 shadow-md">
                  <tr>
                    {["Index", "Account No.", "Amount Paid", "Actions"].map((header) => (
                      <th key={header} className="px-4 md:px-8 py-4 md:py-5 text-left text-sm md:text-base font-bold text-orange-800 dark:text-orange-200 uppercase tracking-wider bg-opacity-100">
                        {header}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700 relative z-0">
                  {payments.map((payment, index) => (
                    <tr
                      ...
                      className={`${selectedCell.row === index ? "bg-orange-200 dark:bg-orange-900" : "hover:bg-gray-50 dark:hover:bg-gray-700/50"} transition-colors`}
                    >
                      <td className="px-4 md:px-8 py-3 md:py-5 text-base md:text-xl font-bold text-gray-500 dark:text-gray-400">{payment.index}</td>
                      <td className="px-4 md:px-8 py-3 md:py-4">
                        <input
                          ...
                          className={`w-full px-3 md:px-6 py-2 md:py-3 rounded-xl border text-base md:text-xl font-bold 
                            ${selectedCell.row === index && selectedCell.column === "accountNo" ? "border-orange-500 ring-2 ring-orange-500/20" : "border-gray-300 dark:border-gray-600"}
                      ...
                      <td className="px-4 md:px-8 py-3 md:py-4">
                        <input
                          ref={(el) => { inputRefs.current[`amountPaid-${index}`] = el; }}
                          type="number"
                          ...
                          onKeyDown={handleKeyDown}
                          className={`w-full px-3 md:px-6 py-2 md:py-3 rounded-xl border text-base md:text-xl font-bold
                            ${selectedCell.row === index && selectedCell.column === "amountPaid" ? "border-orange-500 ring-2 ring-orange-500/20" : "border-gray-300 dark:border-gray-600"}
                            ${payment.isDefaultAmount ? "text-orange-600 dark:text-orange-400" : ""}
                            bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all`}
      ...
      <AccountFinder
        onAccountSelect={(accountNo) => {
          ...
          setTimeout(() => { inputRefs.current[`accountNo-${currentRow}`]?.focus(); }, 0);
        }}
