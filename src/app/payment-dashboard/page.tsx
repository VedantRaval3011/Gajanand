    ...
    <div className={`min-h-screen transition-colors duration-200 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'} ${ubuntu.className} antialiased`}>
      ...
      <div className={`w-full ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
        <div className="max-w-7xl mx-auto px-4 py-4 sm:py-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-center sm:text-left">Payment History Dashboard</h1>
      ...
      <div className="mx-auto px-2 sm:px-4 py-4 sm:py-8">
        ...
        <div className={`mb-4 sm:mb-8 p-4 sm:p-6 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-end gap-4">
            <div className="flex-1">
              <label className="block font-bold text-lg sm:text-xl mb-2 text-orange-500">Select Month</label>
              <input
                type="month"
                value={selectedMonth}
                autoFocus
        ...
        {data.length > 0 ? (
          <div className={`rounded-lg shadow-md overflow-x-auto ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <div className="min-w-[1000px]"> {/* Minimum width container for horizontal scroll */}
              <table className="w-full border-collapse">
                <thead>
                  <tr className={darkMode ? 'bg-gray-700' : 'bg-gray-50'}>
                    <th className={`sticky left-0 p-2 sm:p-4 font-semibold text-left border-b ${darkMode ? 'bg-orange-700' : 'bg-orange-200'} text-sm sm:text-base`}>Account No.</th>
                    {[...Array(daysInMonth)].map((_, i) => (
                      <th key={i} className="p-2 sm:p-4 font-semibold text-center border-b min-w-[50px] sm:min-w-[60px] text-sm sm:text-lg">
                        {i + 1}
                      </th>
                    ))}
                    <th className="p-2 sm:p-4 font-semibold text-center border-b text-sm sm:text-base">Month Total</th>
                    <th className="p-2 sm:p-4 font-semibold text-center border-b text-sm sm:text-base">Overall Total</th>
                  </tr>
                </thead>
                <tbody>
                  {data.map((account, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        `}
                      >
                        <td className={`sticky left-0 p-2 sm:p-4 font-medium border-b hover:text-orange-500 ${darkMode ? 'bg-amber-700' : 'bg-orange-200'} text-sm sm:text-lg`}>
                          {account.accountNo}
                        </td>
                        {[...Array(daysInMonth)].map((_, dayIndex) => {
                          ...
                          return (
                            <td key={dayIndex} className="p-2 sm:p-4 text-center border-b text-sm sm:text-base">
                              {payment ? (
                  ...
                  <tr className={`${darkMode ? 'bg-gray-900' : 'bg-gray-100'} font-bold`}>
                    <td className="sticky left-0 p-2 sm:p-4 border-b bg-orange-300 text-sm sm:text-base">Totals</td>
                    {dailyTotals.map((total, index) => (
                      <td key={index} className="p-2 sm:p-4 text-center border-b font-bold text-sm sm:text-lg text-blue-600 dark:text-blue-400">
          ...
          <div className={`text-center p-4 sm:p-8 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <p className="text-base sm:text-lg font-medium">No payments exist for the selected period.</p>
          </div>
        )}
      </div>
    </div>
